...
CPU_CNT         = 64
RAD_VALS        = [np.load("data/radius_dis.npy")[i] for i in range(CPU_CNT)]
TOPO_VALS       = [np.load("data/topo_dis.npy")[i] for i in range(CPU_CNT)]
DIST_VALS       = [np.load("data/dist_dis.npy")[i] for i in range(CPU_CNT)]
E_DIST_VALS     = [np.load("data/e_dist_dis.npy")[i] for i in range(CPU_CNT)]
TOPO            = {""}
...
def main():
    ...
    for i in range(CPU_CNT):
        for freq_mhz in FREQ_LIST_MHZ:
            ...
            topo        = {"electrode_type":["constant"]*8}
