...
N_P_VALUES   = [9]
CPU_CNT      = 32
V_WRITES     = [[0.02,0.01], [0.01,0.02], [0.04,0.01], [0.01,0.04]]
T_WRITES     = [[1.6,13.9], [13.9,1.6], [0.3,13.9], [13.9,0.3]]
T_WAITS      = [0.1,  0.15,  0.2,  0.3,  0.5,  0.7, 1. ,  1.5 ,  2. ,  3. ,  5. ,  7. , 10. , 15.  , 20. , 30. , 50. , 70. , 100. , 150. , 200. , 300. , 500. , 700. , 1000.]
OUTPUT_DIR   = Path("/scratch/j_mens07/data/2_funding_period/dynamic/DC/two_asym_steps/")
LOG_LEVEL    = logging.INFO
# ────────────────────────────────────────────────────────────────────────────────

def paired_asym_pulse(V1, V2, t1, t2, t_wait, dt, N_electrodes=8, input_pos=0):
    
    # Time intervals
    t1_end      = t1
    t_wait_end  = t1 + t_wait
    t2_end      = t1 + t_wait + t2
    ...
    V_series[pulse1_indices] = V1
    pulse2_indices  = t_series >= t_wait_end
    V_series[pulse2_indices] = V2
...
def main():
    ...
    for N_P in N_P_VALUES:
        ...
        for V1, V2 in V_WRITES:
            for t1, t2 in T_WRITES:
                for t_wait in T_WAITS:
                    time_pts, volt = paired_asym_pulse(V1, V2, t1*1e-9, t2*1e-9, t_wait*1e-9, TIME_STEP, n_elec)
                    args    = (time_pts, volt, topo, OUTPUT_DIR)
                    kwargs  = {
                        'net_kwargs': {'add_to_path' : f"_{V1:.5f}_{V2:.5f}_{t1:.5f}_{t2:.5f}_{t_wait:.5f}"},
