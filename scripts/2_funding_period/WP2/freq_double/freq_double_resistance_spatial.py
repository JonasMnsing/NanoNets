...
def parallel_code(thread, voltages, time_steps, topology_parameter, res_info, eq_steps, folder, np_info, res_info2, T_val, save_th, stat_size, seed):
    ...
    for s in range(stat_size):
        ...
        sim_class = nanonets.simulation(network_topology='cubic', topology_parameter=topology_parameter, folder=folder+f"data/R_{R_val}_d/", res_info=res_info, np_info=np_info, res_info2=res_info2, add_to_path=f'_t{thread}_s{s}', seed=seed)
...
if __name__ == '__main__':
    ...
    folder      = "scripts/2_funding_period/WP2/freq_double/R_spatl/"
