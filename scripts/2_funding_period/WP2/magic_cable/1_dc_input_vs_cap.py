...
def run_simulation(time_steps, voltages, topology_parameter, folder, stat_size, cap):
    ...
    sim_class           = nanonets.simulation(topology_parameter=topology_parameter, folder=folder,
                                              high_C_output=True, np_info2=np_info2, add_to_path=f'_{cap}')
...
if __name__ == '__main__':
    ...
    for i in range(N_processes):
        cap     = cap_vals[i]
        process = multiprocessing.Process(target=run_simulation, args=(time_steps, volt, topology_parameter,
                                                                       folder+"potential/magic_cable/dc_input_vs_cap/", stat_size, cap))
        process.start()
