...
# Simulation Function
def parallel_code(thread, voltages, time_steps, topology_parameter, res_info, eq_steps, folder, np_info, res_info2, T_val, save_th, add_to_path, stat_size, seed):

    target_electrode = len(topology_parameter["e_pos"]) - 1
    
    for s in range(stat_size):

        sim_class = nanonets.simulation(network_topology='cubic', topology_parameter=topology_parameter, folder=folder+"data/", res_info=res_info, np_info=np_info, res_info2=res_info2, add_to_path=add_to_path+f'_t{thread}_s{s}', seed=seed)
...
if __name__ == '__main__':
    ...
    for i in range(N_processes):
        ...
        process = multiprocessing.Process(target=parallel_code, args=(i, voltages, time_steps, topology_parameter, res_info, eq_steps, folder,
                                                                      np_info, res_info2, T_val, save_th, add_to_path, stat_size, seed))
        process.start()