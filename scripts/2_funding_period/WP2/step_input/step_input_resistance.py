...
# Simulation Function
def parallel_code(thread, voltages, time_steps, topology_parameter, res_info, res_info2, eq_steps, folder, np_info, T_val, save_th, stat_size):
    ...
    for s in range(stat_size):
        ...
        sim_class = nanonets.simulation(network_topology='cubic', topology_parameter=topology_parameter, folder=folder+f"data/R_{R_val}/r1/", res_info=res_info, res_info2=res_info2, np_info=np_info, add_to_path=f'_t{thread}_s{s}')
...
if __name__ == '__main__':
    ...
    res_info2   = {
        "R" : 50,
        "np_index" : np.random.choice(np.arange(1,48), 9, replace=False)
    }
    ...
    for i in range(N_processes):
        ...
        process = multiprocessing.Process(target=parallel_code, args=(i, voltages, time_steps, topology_parameter, res_info, res_info2, eq_steps, folder,
                                                                      np_info, T_val, save_th, stat_size))
        process.start()
