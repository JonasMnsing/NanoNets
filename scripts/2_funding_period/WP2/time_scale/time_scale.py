...
def run_simulation(thread, time_steps, voltages, radius, folder, stat_size, topology_parameter, np_info2):
...
if __name__ == '__main__':
    ...
    for N_p in N_p_vals:
        ...
        for i in range(N_processes):
            ...
            np_info2 = {
                'np_index'      : [topology_parameter["Nx"]], 
                'mean_radius'   : radius,
                'std_radius'    : 0.0
            }
            procs   = []
            process = multiprocessing.Process(target=run_simulation, args=(i, time_steps, voltages, radius, folder, stat_size, topology_parameter, np_info2))
            process.start()
    ...
    #         radius  = radius_vals[i]
    #         np_info2 = {
    #             'np_index'      : [int(topology_parameter["Nx"]**2)], 
    #             'mean_radius'   : radius,
    #             'std_radius'    : 0.0
    #         }
    #         procs   = []
    #         process = multiprocessing.Process(target=run_simulation, args=(i, time_steps, voltages, radius, N_p, folder, stat_size, topology_parameter, np_info2))
    #         process.start()
