...
def run_sim(thread, x, params, rows, time_steps, topology, path, amplitude):
    ...
    for n, par in enumerate(params_new):
        ...
        for i, p in enumerate(par):
            voltages[:,i+1] = p #amplitude*np.cos(p*time_steps*1e8)

        # Run Training
        sim_class   = nanonets.simulation(topology_parameter=topology, folder=path, seed=0, add_to_path=f'_{thread}_{n}')
        sim_class.run_var_voltages(voltages=voltages, time_steps=time_steps, target_electrode=7, stat_size=500)
...
topology_parameter  = {
    ...
    "Ny"                : N_p,
    "Nz"                : 1,
    "e_pos"             : [[(N_p-1)//2,0,0],[0,0,0],[0,(N_p-1)//2,0],
                           [N_p-1,0,0],[0,N_p-1,0],[N_p-1,(N_p-1)//2,0],
...
p_range     = [[-0.05,0.05] for _ in range(N_controls)]
