...
stat_size           = 500
eq_steps            = 0
path                = "scripts/2_funding_period/WP2/training/data/m_glass/"
...
# M Glass Data
data        = np.loadtxt("scripts/2_funding_period/WP2/training/m_data.csv", delimiter=',')
x_vals      = data[:,0]
x_vals      = 0.05*(x_vals - np.mean(x_vals))/np.std(x_vals)
y_target    = data[:,1]
time_step   = 1e-10
